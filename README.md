...
```bash
docker pull ghcr.io/chinmaykrishnroy/monorally:1.2.2
docker run --rm -p 8787:8787 --env-file .env ghcr.io/chinmaykrishnroy/monorally:1.2.2
```
...
GitHub Actions validates every push and pull request with syntax checks, unit tests, Chromium end-to-end tests, a WebSocket smoke test, and a Docker build. Pushing a version tag such as `v1.2.2` repeats those gates, then publishes multi-architecture images and creates the GitHub Release.
