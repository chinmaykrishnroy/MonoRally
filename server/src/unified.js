...
export async function startUnifiedNode(infra, options = {}) {
  ...
  const endRoomByPresence = (room, winner) => {
    room.status = "ended";
    room.winner = winner;
  ...
  async function handleDrain() {
    if (draining) return;
    draining = true;
    console.log("[unified] Graceful draining initiated...");
    if (infra.workerRegistry) await infra.workerRegistry.markWorkerDraining(WORKER_ID);
    if (rooms.size === 0) {
      await stop();
      return;
    }
  ...
  function handleMessage(client, msg) {
    ...
    if (msg.t === "hello") {
      ...
      send(client, { t: "hello", id: client.id, name: client.name, port, protocol: client.protocol });
    }
  ...
  await new Promise((resolve) => {
    server.listen(port, () => {
      console.log(`[unified] MonoRally unified node running at http://localhost:${port}`);
      console.log(`[unified] Allowed origins: ${ALLOWED_ORIGINS.join(", ")}`);
      resolve();
    });
  });
  ...
  const stop = async () => {
    ...
    if (infra.workerRegistry) await infra.workerRegistry.unregisterWorker(WORKER_ID);
